...
const AffiliateControlPanel = ({ onRefresh }: Props) => {
  ...
  const toggleAffiliate = async () => {
    ...
    if (error) {
      toast.error('Failed to toggle affiliate network');
    } else {
      setEnabled(newVal);
      toast.success(`Affiliate Network ${newVal ? 'Enabled' : 'Disabled'}`);
      onRefresh();
    }
  };

  const addProduct = async () => {
    if (!newProduct.title || !newProduct.image_url || !newProduct.external_link) {
    ...
    if (error) {
      ...
      setNewProduct({ title: '', price_cents: 0, image_url: '', external_link: '' });
      fetchData();
    }
  };
    ...
    <div className="bg-[#1a1a2e] rounded-lg border border-gray-800 p-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-bold text-white">Affiliate Network</h2>
        </div>
      ...
      <div className="grid grid-cols-2 gap-2 mb-4">
        ...
        <Input
          ...
          onChange={(e) =>
            setNewProduct({ ...newProduct, price_cents: parseInt(e.target.value) || 0 })
          }
          className="bg-gray-900 border-gray-700 text-white text-xs"
        />
        <Input
      ...
      <Button onClick={addProduct} size="sm" className="mb-4 bg-orange-500 hover:bg-orange-600 text-white">
        <Plus className="h-3 w-3 mr-1" /> Add Affiliate Product
      </Button>
      ...
      <div className="space-y-2 max-h-60 overflow-y-auto">
        {loading ? (
          ...
          <p className="text-xs text-gray-500">No affiliate products yet.</p>
        ) : (
          products.map((p) => (
            <div
              key={p.id}
              className="flex items-center justify-between bg-gray-900/50 rounded-md px-3 py-2"
            >

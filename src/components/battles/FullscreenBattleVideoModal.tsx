...
const renderVideoPlayer = (barber: BarberData) => {
  // Priority: submission video > live stream > featured video > placeholder
  if (barber.submission?.youtube_vod_url) {
    return (
      <YouTubeStreamPlayer
        videoUrl={barber.submission.youtube_vod_url}
  ...
  if (barber.isLive && barber.submission?.media_url) {
    return (
      <VideoPlayer
...
export const FullscreenBattleVideoModal = ({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent 
        className="max-w-[100vw] max-h-screen w-screen h-screen p-0 bg-black border-0"
        onEscapeKeyDown={onClose}
      >
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-50 bg-black/70 hover:bg-black/90 rounded-full p-2 transition-colors"
        >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 h-full lg:h-screen overflow-y-auto lg:overflow-hidden gap-0">
          ...
          <div className="relative h-screen lg:h-full flex flex-col bg-black">
            ...
            <div className="bg-gradient-to-t from-black via-black/95 to-transparent p-4 lg:p-6 space-y-3 lg:space-y-4">
              ...
              <div className="flex flex-wrap gap-2 lg:gap-3">
                <Button 
                  onClick={() => onFollow(barber1.user_id)}
                  size="sm"
                ...
                <Button 
                  onClick={() => onLike(barber1.user_id)}
                  size="sm"
                  variant="secondary"
                  className="flex-1 lg:flex-initial"
                >
                ...
                <Button 
                  onClick={() => onVote(barber1.user_id)}
                  disabled={userVotedFor !== null}
                  size="sm"
                  variant={userVotedFor === barber1.user_id ? "secondary" : "default"}
                  className="flex-1 lg:flex-initial"
                >
                  <ThumbsUp className="w-4 h-4 mr-2" />
                  {userVotedFor === barber1.user_id ? "Voted" : "Vote"}
                </Button>
                <Button 
                  onClick={() => onDonate(barber1.user_id, barber1.name)}
                  size="sm"
                  variant="outline"
                  className="flex-1 lg:flex-initial"
                >
          ...
          <div className="relative h-screen lg:h-full flex flex-col bg-black lg:border-l border-white/10">
            ...
            <div className="bg-gradient-to-t from-black via-black/95 to-transparent p-4 lg:p-6 space-y-3 lg:space-y-4">
              ...
              <div className="flex flex-wrap gap-2 lg:gap-3">
                <Button 
                  onClick={() => onFollow(barber2.user_id)}
                  size="sm"
                  className="flex-1 lg:flex-initial"
                >
                  <UserPlus className="w-4 h-4 mr-2" />
                ...
                <Button 
                  onClick={() => onLike(barber2.user_id)}
                  size="sm"
                  variant="secondary"
                  className="flex-1 lg:flex-initial"
                >
                ...
                <Button 
                  onClick={() => onVote(barber2.user_id)}
                  disabled={userVotedFor !== null}
                  size="sm"
                  variant={userVotedFor === barber2.user_id ? "secondary" : "default"}
                  className="flex-1 lg:flex-initial"
                >
                  <ThumbsUp className="w-4 h-4 mr-2" />
                  {userVotedFor === barber2.user_id ? "Voted" : "Vote"}
                </Button>
                <Button 
                  onClick={() => onDonate(barber2.user_id, barber2.name)}
                  size="sm"
                  variant="outline"
                  className="flex-1 lg:flex-initial"
                >
